...
let sort { sort = sort; _ } = sort
let startp { startp = startp; _ } = startp
let endp { endp = endp; _ } = endp
let location { startp = startp; endp = endp; _ } = (startp, endp)

(* ---------------------------------------------------------------------------- *)

(* Comparison. *)

let compare { name = name1; sort = (sort1, _); _ } { name = name2; sort = (sort2, _); _ } =
  let c = compare (sort1 : int) sort2 in
  if c = 0 then
